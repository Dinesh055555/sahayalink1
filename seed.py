...
CREATE TABLE worklist(
  ...
  consult_status TEXT DEFAULT 'awaiting',  -- awaiting | completed  (the teleconsult stage)
  doctor_id INTEGER, hospital_id INTEGER, consult_time TEXT);

CREATE TABLE hospitals(
  id INTEGER PRIMARY KEY, name TEXT, district TEXT, block TEXT, hosp_type TEXT);

CREATE TABLE doctors(
  id INTEGER PRIMARY KEY, name TEXT, specialty TEXT, hospital_id INTEGER,
  teleconsult_mode TEXT);
...
# ---- hospitals & doctors (the teleconsultation network) ----
DISTRICTS = ['Mahabubnagar', 'Nalgonda', 'Sangareddy']
SPECIALTIES = ['General Medicine', 'Cardiology', 'Diabetology / Endocrinology',
               'General Physician', 'Internal Medicine', 'Family Medicine']
DR_FIRST = ['Ravi','Sneha','Kiran','Anita','Prasad','Meena','Suresh','Deepa','Arjun',
            'Latha','Naveen','Kavya','Rahul','Divya','Mohan','Swathi']
hid = 1; did = 1
for d in DISTRICTS:
    # 2 hospitals per district: 1 district hospital + 1 community health centre hub
    for htype, hname in [('District Hospital', f'{d} District Hospital'),
                         ('Teleconsult Hub', f'{d} CHC Teleconsult Hub')]:
        c.execute('INSERT INTO hospitals VALUES(?,?,?,?,?)', (hid, hname, d, '', htype))
        # 3 doctors per hospital across specialties
        for k in range(3):
            nm = 'Dr. ' + DR_FIRST[(did-1) % len(DR_FIRST)]
            sp = SPECIALTIES[(did-1) % len(SPECIALTIES)]
            mode = 'Video' if k % 2 == 0 else 'Audio'
            c.execute('INSERT INTO doctors VALUES(?,?,?,?,?)', (did, nm, sp, hid, mode))
            did += 1
        hid += 1
print(f'Seeded {hid-1} hospitals, {did-1} doctors')
...
for r in cases:
    ...
    # Consult stage: ~1 in 4 patients still awaits the teleconsult (the CHO's bridge queue);
    # the rest already had it and are now in follow-up, with a doctor/hospital on record.
    hosp = c.execute('SELECT id FROM hospitals WHERE district=? ORDER BY id LIMIT 1', (cd,)).fetchone()
    hosp_id = hosp[0] if hosp else None
    doc = c.execute('SELECT id FROM doctors WHERE hospital_id=? ORDER BY id LIMIT 1', (hosp_id,)).fetchone() if hosp_id else None
    doc_id = doc[0] if doc else None
    awaiting = (placed % 4 == 0)  # every 4th patient awaits consult
    cstat = 'awaiting' if awaiting else 'completed'
    c.execute('''INSERT INTO worklist(work_date,episode_id,patient_ref,village,district,priority,risk_pct,
        ...
        consult_status,doctor_id,hospital_id,consult_time)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
        (today, r['episode_id'], r['patient_ref'], cv, cd, r['priority'], r['risk_pct'],
         ...
         cstat, None if awaiting else doc_id, None if awaiting else hosp_id,
         None if awaiting else today))
    placed += 1
