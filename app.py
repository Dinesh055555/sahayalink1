...
def worklist(user=Depends(user_from_token)):
    ...
    rows = con.execute('''SELECT w.*, d.name doctor_name, d.specialty, h.name hospital_name
        FROM worklist w LEFT JOIN doctors d ON d.id=w.doctor_id
        LEFT JOIN hospitals h ON h.id=w.hospital_id
        WHERE w.assigned_user_id=? AND w.work_date=?
        ORDER BY CASE w.priority WHEN 'Critical' THEN 0 ELSE 1 END, w.risk_pct DESC''',
...
# ---------- CHO: teleconsultation bridge ----------
@app.get('/api/cho/network')
def cho_network(user=Depends(user_from_token)):
    """Hospitals and their doctors, for the CHO to assign patients to."""
    if user['role'] not in ('CHO','SUPERVISOR'):
        raise HTTPException(403,'CHO access only')
    con=db()
    hosp=con.execute('SELECT * FROM hospitals ORDER BY district,name').fetchall()
    docs=con.execute('SELECT * FROM doctors ORDER BY hospital_id,name').fetchall()
    con.close()
    return {'hospitals':[dict(h) for h in hosp],'doctors':[dict(d) for d in docs]}

@app.get('/api/cho/consults')
def cho_consults(status: str = 'awaiting', user=Depends(user_from_token)):
    """Patients awaiting a teleconsult (the CHO's bridge queue), or completed ones."""
    if user['role'] not in ('CHO','SUPERVISOR'):
        raise HTTPException(403,'CHO access only')
    con=db(); today=date.today().isoformat()
    rows=con.execute('''SELECT w.*, d.name doctor_name, d.specialty, h.name hospital_name
        FROM worklist w LEFT JOIN doctors d ON d.id=w.doctor_id
        LEFT JOIN hospitals h ON h.id=w.hospital_id
        WHERE w.work_date=? AND w.consult_status=?
        ORDER BY CASE w.priority WHEN 'Critical' THEN 0 ELSE 1 END, w.risk_pct DESC''',
        (today,status)).fetchall()
    con.close()
    return {'status':status,'count':len(rows),'patients':[dict(r) for r in rows]}

@app.get('/api/cho/summary')
def cho_summary(user=Depends(user_from_token)):
    if user['role'] not in ('CHO','SUPERVISOR'):
        raise HTTPException(403,'CHO access only')
    con=db(); today=date.today().isoformat()
    a=con.execute("SELECT COUNT(*) n FROM worklist WHERE work_date=? AND consult_status='awaiting'",(today,)).fetchone()['n']
    d=con.execute("SELECT COUNT(*) n FROM worklist WHERE work_date=? AND consult_status='completed'",(today,)).fetchone()['n']
    by_hosp=con.execute('''SELECT h.name hospital, COUNT(w.id) consults
        FROM worklist w JOIN hospitals h ON h.id=w.hospital_id
        WHERE w.work_date=? AND w.consult_status='completed' GROUP BY h.id ORDER BY consults DESC''',(today,)).fetchall()
    by_district=con.execute('''SELECT district, SUM(consult_status='awaiting') awaiting,
        SUM(consult_status='completed') completed FROM worklist WHERE work_date=? GROUP BY district''',(today,)).fetchall()
    con.close()
    tot=a+d
    return {'awaiting':a,'completed':d,'completion_rate':round(100*d/tot) if tot else 0,
            'by_hospital':[dict(r) for r in by_hosp],'by_district':[dict(r) for r in by_district]}
...
@app.post('/api/cho/complete-consult')
def complete_consult(body: Consult, user=Depends(user_from_token)):
    """CHO bridges a patient to a doctor+hospital and marks the teleconsult stage complete.
    The remaining tasks (medicine, review, test) stay open and flow to the ASHA."""
    if user['role'] != 'CHO':
        raise HTTPException(403,'Only a CHO can complete a teleconsultation')
    con=db()
    w=con.execute('SELECT * FROM worklist WHERE id=?',(body.worklist_id,)).fetchone()
    if not w: con.close(); raise HTTPException(404,'Patient not found')
    now=datetime.now().isoformat(timespec='minutes')
    con.execute('''UPDATE worklist SET consult_status='completed', doctor_id=?, hospital_id=?, consult_time=?
        WHERE id=?''',(body.doctor_id,body.hospital_id,now,body.worklist_id))
    con.execute('INSERT INTO action_log(worklist_id,user_id,outcome,note,ts) VALUES(?,?,?,?,?)',
        (body.worklist_id,user['id'],'consult_completed',f'doc {body.doctor_id} @ hosp {body.hospital_id}',now))
    d=con.execute('SELECT name,specialty FROM doctors WHERE id=?',(body.doctor_id,)).fetchone()
    h=con.execute('SELECT name FROM hospitals WHERE id=?',(body.hospital_id,)).fetchone()
    con.commit(); con.close()
    return {'ok':True,'doctor':d['name'] if d else '','hospital':h['name'] if h else '','when':now}
